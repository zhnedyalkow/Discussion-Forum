        ...
        .post('/cat/:id', async(req, res) => {
            ...
            try {
                ...
                try {
                    ...
                    await controller
                        .addAnswer({ answerContent: content, PostId: +getId[0].dataValues.id, UserId: who[0].dataValues.id });
                    console.log(content);
                } catch (err) {
                    const plit = await controller
                        .createPost({ title: newPost, content: newContent, ThreadId: +id, UserId: who[0].dataValues.id });

                    await controller
                        .addAnswer({ answerContent: newContent, PostId: +plit[0].dataValues.id, UserId: who[0].dataValues.id });
                }
                ...
                try {
                    await controller
                        .addAnswer({ answerContent: content, PostId: +getId[0].dataValues.id, UserId: 4 });
                } catch (err) {
                    const plit = await controller
                        .createPost({ title: newPost, content: newContent, ThreadId: +id, UserId: 4 });
                    await controller
                        .addAnswer({ answerContent: newContent, PostId: +plit[0].dataValues.id, UserId: 4 });
                }
                await res.redirect('/cat/' + id);
            }
        });
